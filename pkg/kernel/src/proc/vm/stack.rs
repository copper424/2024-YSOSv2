...
impl Stack {
    ...
    pub fn from_range(range: PageRange<Size4KiB>, page_size: u64) -> Self {
        Self {
            range,
            usage: page_size,
        }
    ...
    pub fn handle_page_fault(
        ...
        trace!("Page fault on stack: {:#x}", addr.as_u64());
    ...
    pub fn cal_offset(&self, other: &Self) -> u64 {
        let curr_bot = self.range.start.start_address().as_u64();
        let other_bot = other.range.start.start_address().as_u64();
        curr_bot - other_bot
    }

    pub fn fork(
        &self,
        page_table_mapper: MapperRef,
        frame_allocator: FrameAllocatorRef,
        stack_offset_count: u64,
    ) -> Self {
        // FIXME: alloc & map new stack for child (see instructions)
        let origin_stack_base = self.range.start.start_address().as_u64();
        let mut new_stack_base = origin_stack_base - stack_offset_count;
        while elf::map_range(
            new_stack_base,
            self.usage,
            page_table_mapper,
        ...
        memory::clone_range(origin_stack_base, new_stack_base, self.usage as usize);

        // FIXME: return the new stack
        let new_bot_page = Page::containing_address(VirtAddr::new(new_stack_base));
        let new_end_page = new_bot_page + self.usage;
        Self::from_range(
            PageRange {
                start: new_bot_page,
                end: new_end_page,
